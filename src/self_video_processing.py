...
# data_dir = '../data/slipper3_3'
data_dir = '../data/cup_holder2_2'
...
# slipper
# v_xstart, v_ystart = 100, 600
# v_xend, v_yend = 400, 1250

# cupboard
v_xstart, v_ystart = 50, 600
v_xend, v_yend = 290, 1250

v_h = v_xend - v_xstart
v_w = v_yend - v_ystart

# slipper
# h_xstart, h_ystart = 890, 400
# h_xend, h_yend = 1280, 1400

# cupboard
h_xstart, h_ystart = 900, 400
h_xend, h_yend = 1080, 1400
...
for i in range(num_imgs):
    ...
    if i % vis_freq == 0 and vis and i >= 80 and i <= 310:
        if i in frame2skip:
            continue
        print("i = ", i)
        if a_v != 0:
...
# Load the extrinsics
ext_calib = np.load('./cam/extrinsic_calib.npz')
R_h, t_h = ext_calib['rmat_h'], ext_calib['tvec_h']
R_v, t_v = ext_calib['rmat_v'], ext_calib['tvec_v']

int_calib = np.load('./cam/intrinsic_calib.npz')
...
obj_xstart, obj_ystart = 250, 550
obj_xend, obj_yend = 900, 1250

# obj_xstart, obj_ystart = 300, 620
# obj_xend, obj_yend = 840, 1180
...
ax.scatter(pts_filtered[:, 0], pts_filtered[:, 1], pts_filtered[:, 2],
           s=0.05, c=colors_filtered, cmap='gray')
